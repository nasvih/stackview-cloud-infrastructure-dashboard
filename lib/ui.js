...
/* Dates follow the language on <html>: Arabic month names, Latin digits, so
   a date still lines up with the money and the percentages in a table. Money
   and units are deliberately left alone — GB, vCPU, ms and % are read as
   written in both directions. */
export const dateLocale = () => (document.documentElement.lang === 'ar' ? 'ar-u-nu-latn' : 'en-GB');

/* The relative words come from the app's dictionary; these are the English
   defaults so lib/ui.js still stands on its own. */
let AGO = {
  now: 'just now',
  mins: (n) => `${n}m ago`,
  hours: (n) => `${n}h ago`,
...
export function setAgoStrings(o) { AGO = Object.assign({}, AGO, o); }
...
export function ago(d) {
  ...
  if (s < 3600) return AGO.mins(Math.floor(s / 60));
  if (s < 86400) return AGO.hours(Math.floor(s / 3600));
  if (s < 2592000) return AGO.days(Math.floor(s / 86400));
  return fmtDate(d);
}
...
/* ---------- modal ---------- */
/* The three words the runtime says on its own. The app hands over its own
   pair at start-up so a modal reads in the chosen language. */
let UI = { cancel: 'Cancel', confirm: 'Confirm', close: 'Close' };
export function setUiStrings(o) { UI = Object.assign({}, UI, o); }
...
export function modal({ title, body, actions = [], width }) {
  ...
  box.appendChild(h('div', { class: 'modal__head' },
    h('h3', {}, title || ''),
    h('button', { class: 'btn btn--ghost btn--icon', onclick: close, 'aria-label': UI.close, title: UI.close, html: '<svg viewBox="0 0 20 20"><path d="M5 5l10 10M15 5L5 15"/></svg>' })));
